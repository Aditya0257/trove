...
## D24 - Mandatory email verification (OTP) before the admin approval gate

- **Decision:** A new sign-up starts `unverified` and must confirm a 6-digit code
  emailed to it (`POST /api/auth/verify-email`) before anything else. Only once the
  email is verified does the approval gate apply: with an admin configured and the
  account is not itself an admin, it becomes `pending` and the admins are emailed; an
  admin approves it to `active`. Status lifecycle: `unverified -> pending -> active`
  (or `rejected`). There is no skip.
- **Why:** an unreachable email can never receive a password reset or a reminder, so a
  real, owned address is a precondition, not an optional extra. Verifying first also
  keeps junk sign-ups off the admin's approval queue.
- **Contract:** `email_verification` table (V24): one active code per user, stored only
  as a SHA-256 hash, 15-minute expiry, locks after 5 wrong attempts. Sent via Brevo
  (D-email). `register` returns `202` with `status:"unverified"` and no token.
- **Touches:** `auth/EmailVerificationService`, `EmailVerification(Repository)`,
  `AuthController` (register/verify-email/resend-verification), `UserService`
  (`finishVerification`); web `features/auth/verify.ts` + 3-step sign-up indicator.
- **Status:** active - verified live against Brevo + Neon.

## D25 - Admins are a config allow-list, still with no privileged DB flag

- **Decision:** admin identity is the union of `trove.admin.email` (single, legacy) and
  `trove.admin.emails` (comma/space list). One or more emails, decided purely by
  configuration; blank = open registration. There is still no `is_admin` column.
- **Why:** lets a deployment name a backup or second admin by editing an env var and
  restarting, with no code change and nothing in the database to leak or misassign.
  Keeps the original "no privileged flag" property of D (admin approval).
- **Touches:** `auth/UserService` (`parseAdminEmails`, `isAdmin`); `.env(.example)`,
  `operations/configuration.md`.
- **Status:** active.

## D26 - Self-service account settings; account deletion is admin-only and irreversible

- **Decision:** users manage their own profile at `/account` (display name, photo,
  password change re-checking the current one, email change confirmed by an OTP to the
  new address, TOTP). Deleting an account is NOT self-service: it is an admin-only,
  irreversible action that purges every document in the user's spaces from R2, Google
  Drive and the DB, then clears the remaining rows and the account. Guards: an admin
  cannot delete themselves or another admin, and must re-type the target's exact email
  (checked server-side).
- **Why:** this vault optimises for zero data loss, so the one operation that destroys a
  person's vault is deliberately privileged and heavily guarded, not a button any user
  can hit by accident. The append-only B2 mirror still keeps an archival copy.
- **Contract:** `app_user.avatar_key` + `pending_email` (V25); avatars live in R2
  (never the DB) and are served as short-lived presigned URLs. `AccountController`
  (`/api/account/*`), `AdminController` (`GET /users`, `POST /users/{id}/delete`),
  `AccountDeletionService`.
- **Touches:** web `features/account/account.ts`, `core/avatar.ts`, nav avatar; the old
  `/security` page now redirects to `/account`.
- **Status:** active - delete flow and guards verified live.

## D27 - Read paths are paginated/scoped server-side, not fetched whole

- **Decision:** list endpoints return one page and never the whole set. Documents:
  `GET /api/documents?page=&size=` returns a page with the total in an `X-Total-Count`
  header (`size=0` = all, back-compatible); the default no-category list excludes the
  `email` category at the DB level. Mail: `GET /api/mail` groups emails into threads in
  the database (`GROUP BY extra->>'mailBundleId'`), paginated, with distinct
  account/topic/address facets; a thread opens via `GET /api/documents/mail-bundle`.
  Reminders carry each linked document's filename (batch `findAllById`) so the page
  needs no document fetch; the picker's list loads lazily. Search already ran a bounded
  DB query.
- **Why:** the app targets a small, trusted group on free-tier infra. Paginating keeps
  the stateless app server's memory and payload proportional to one page, which is
  lighter on the Oracle Always-Free VM and Neon than shipping the whole set and grouping
  in the browser. It also stops other tabs (Mail, Reminders) from re-fetching everything.
- **Touches:** `document/DocumentController`+`DocumentService`+`DocumentRepository`,
  `mail/*` (new), `reminder/*`, `common/security/SecurityConfig` (expose `X-Total-Count`);
  web `doc-list.ts`, `mail.ts`, `mail-detail.ts`, `reminders.ts`, `api.service`.
- **Status:** active - verified live (page counts, no overlap, facets, thread grouping).

## D28 - Multilingual vault chat by normalizing the query, not swapping the embedder

- **Decision:** a Hindi/Hinglish question is rewritten to a concise English search query
  before it is embedded; retrieval, reminder detection and the grounded answer all run
  on the normalized query, while the original still drives nothing user-visible beyond
  it. The English-only embedder (`bge-base-en`) and the English document corpus are kept
  as-is.
- **Why:** the documents (Indian bills, receipts) are printed in English, so the
  mismatch is only on the query side. Fixing the query is far cheaper than a multilingual
  embedder, which would force a 1024-dim vector column and a full re-index for a corpus
  that is already English. A cheap heuristic passes plain English through untouched (no
  model call); only a likely non-English question spends one tiny router-model call, and
  any failure falls back to the raw question so retrieval never gets worse.
- **Touches:** `chat/QueryNormalizer` (new), `chat/VaultChatService`.
- **Status:** active - verified live (Hinglish query went from 0 sources to a correct,
  cited answer).
